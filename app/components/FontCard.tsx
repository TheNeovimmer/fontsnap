...
export default function FontCard({
  ...
  return (
    <motion.article
      ...
      className="specimen-card group relative flex flex-col rounded-[20px] border border-[var(--scan-line-2)] bg-white dark:bg-[var(--surface-1)] transition-[border-color,box-shadow] duration-[160ms]"
    ...
    >
      ...
      <header className="flex items-start justify-between gap-5 px-6 pt-5 md:px-9 md:pt-7">
        ...
        <button
          ...
          className={`scan-focusable flex h-9 shrink-0 items-center gap-1.5 rounded-full border px-3.5 text-[12.5px] font-semibold transition-[background-color,border-color,color,box-shadow] duration-[140ms] active:scale-[0.97] disabled:cursor-not-allowed ${
            !canDownload
              ? 'border-[var(--scan-line-1)] bg-white dark:bg-slate-800 text-[var(--scan-ink-4)] opacity-70'
              : downloadState === 'success'
                ? 'border-[#a7e3c9] dark:border-[#1a5c3a] bg-[#eafaf3] dark:bg-[#0d2818] text-[var(--success)]'
                : downloadState === 'error'
                  ? 'border-[#f4c3c7] dark:border-[#5a2024] bg-[#fdf1f2] dark:bg-[#2a1012] text-[var(--danger)]'
                  : downloadState === 'loading'
                    ? 'border-[var(--scan-accent)] bg-[var(--scan-accent)] text-white'
                    : 'border-[var(--scan-line-1)] bg-white dark:bg-slate-800 text-[var(--scan-ink-2)] group-hover:border-[var(--scan-accent)] group-hover:bg-[var(--scan-accent)] group-hover:text-white hover:!border-[var(--scan-accent-deep)] hover:!bg-[var(--scan-accent-deep)] hover:!text-white'
          }`}
        >
          {downloadState === 'loading' ? (
      ...
      <AnimatePresence initial={false}>
        {showAlternatives && altState !== 'idle' && altState !== 'loading' && (
          ...
          >
            <div className="border-t border-[var(--scan-line-3)] bg-[#fafcff] dark:bg-[var(--surface-2)] px-6 py-4 md:px-9">
                  ...
                  {alternatives.map((alt, altIndex) => (
                    <li key={`${alt.family}-${altIndex}`}>
                      <a
                        ...
                        className="scan-focusable flex h-full items-center justify-between gap-3 rounded-lg border border-transparent px-3 py-2.5 transition-colors duration-[120ms] hover:border-[#dbe7f8] dark:hover:border-slate-700 hover:bg-white dark:hover:bg-slate-800"
                      >
